...
/*  jl_task_spawn() -- create a task for `f(arg)` and enqueue it for execution
 ...
 */
JL_DLLEXPORT jl_task_t *jl_task_spawn(jl_value_t *_args, int8_t sticky, int8_t detach)
{
    jl_ptls_t ptls = jl_get_ptls_states();
...
/*  jl_task_parfor() -- spawn multiple tasks for a parallel loop
 ...
 */
JL_DLLEXPORT jl_task_t *jl_task_parfor(jl_value_t *_args, int64_t count, jl_value_t *_rargs)
{
    jl_ptls_t ptls = jl_get_ptls_states();
