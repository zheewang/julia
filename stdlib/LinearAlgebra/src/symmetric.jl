...
@inline function getindex(A::Symmetric, i::Integer, j::Integer)
    ...
    @inbounds if (A.uplo == 'U') == (i < j)
        return A.data[i, j]
    elseif i == j
        return symmetric(A.data[i, j], Symbol(A.uplo))
    else
...
@inline function getindex(A::Hermitian, i::Integer, j::Integer)
    ...
    @inbounds if (A.uplo == 'U') == (i < j)
        return A.data[i, j]
    elseif i == j
        return hermitian(A.data[i, j], Symbol(A.uplo))
    else
...
function Matrix(A::Symmetric)
    ...
    for i = 1:size(A, 1)
        B[i,i] = symmetric(B[i,i], Symbol(A.uplo))
    end
    return B
end
function Matrix(A::Hermitian)
    ...
    for i = 1:size(A, 1)
        B[i,i] = hermitian(B[i,i], Symbol(A.uplo))
    end
    return B
end
