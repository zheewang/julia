...
endof(s::SecureString) = endof(s.string)
length(s::SecureString) = length(s.string)
sizeof(s::SecureString) = sizeof(s.string)
next(s::SecureString, i::Int) = next(s.string, i)
